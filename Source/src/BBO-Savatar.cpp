                    ...
                    if (chance > 20)
                    {
								...
								|| iBlade->damageDone > 71))// or somethtign else with dam 72 or greater
						{
							...
							if (dustTotal>4) // if it has enough dusts on it
							{
								...
								int doparticle = 0;
								if (iBlade->numOfHits > 12000 && ((iBlade->numOfHits - 12000) % 1250 == 0)) // particle for every percentage added
									doparticle = 1;
								if (iBlade->numOfHits == 12000)												// initial partical
									doparticle = 2;
								if (iBlade->numOfHits == 55000)											// old cap for normal
									doparticle = 3;
								if (iBlade->numOfHits == 130750)											// 100%
									doparticle = 10;
								if (doparticle > 0)
								{
...
//				float tempChall = challenge;
				...
				{   // nomral dismantle, even if it is a scythe. the staff is lost eiter way.
					...
					dustReturned = Bracket((int)(work / 10 / challenge * (ib->tame - 15)),
						(int)((ib->tame - 15)*0.3f), (int)((ib->tame - 15)));  // can get 100% of gold back, and ONLY gold.
					dustTotal += ib->tame;
					if (dustReturned >= 1)
					{
					...
					int chance = 0;
					int chance2 = 0;
					if (ib->numOfHits >= 12000)
						chance = ((ib->numOfHits - 12000) / 1250) + 5;
					...
					if (dustReturned > 0 && ib->numOfHits >= 12000 && gotDust && chance2  && damDone >= 72)
					{
					...
					else if (bboServer->enable_cheating!=0) // in cheaters mode, also give old formula a chance
					{
						...
						if (dustReturned > 0 && ib->numOfHits >= 12000 && gotDust && chance == 1 && originalIngotCount >= 96) //cheezable with tin tachi made from unages GS and few dusts.
						{
							int type = INGR_WHITE_SHARD + (rand() % 8);
							io = new InventoryObject(
								INVOBJ_INGREDIENT, 0, dustNames[type]);
