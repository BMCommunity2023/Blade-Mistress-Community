...
//						tempMob->bladeParticleAmount[0] = 1;
					...
					tempMob->bladeParticleAmount[DRAGON_TYPE_GOLD] = messBladeDescPtr->gold;
					...
					tempMob->bladeParticleColor[DRAGON_TYPE_GOLD] = D3DCOLOR_ARGB(55, 255, 215, 0);
...
void BBOClient::DrawBladeEffect(BBOMob *mob)
{
	...
	if (mob->totalBPA && BLADE_TYPE_STAFF1 > mob->bladeType)
	{
		for (int i = 0; i < (1 + mob->totalBPA/25) && i < 5 && (mob->animCounter%3); ++i)
		{
			...
			int j = 0;
			for (; j < 6 && colorCount > mob->bladeParticleAmount[j]; ++j)
