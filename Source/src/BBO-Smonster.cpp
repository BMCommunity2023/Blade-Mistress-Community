...
//				}
				if (controllingAvatar) //if i'm controlled
				{
					...
					if ((ss->WhatAmI()==SPACE_GROUND) && (controllingAvatar->BeastStat() > 9)) // if controlled by a BeastMaster and on main map
					{
						...
						while (badspawn)
						{
							badspawn = !ss->CanMove(randomX, randomY, randomX, randomY); // turns false if spawnw location is valid
						...
						monster->cellY = monster->targetCellY = monster->spawnY = randomX;
						monster->SpecialAI = TRUE;										  // new Unicorn has AI.
						ss->mobList->Add(monster);
						// update location to save later
						bboServer->UnicornLocations[subType][0] = randomX;
						bboServer->UnicornLocations[subType][1] = randomY;
