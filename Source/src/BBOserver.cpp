    ...
    */
	...
	// first check for file
	{
		...
		if (fp2)
		{
			...
			int randomY = rand() % 254;
			bool badspawn = TRUE;
			while (badspawn)
			{
				badspawn = !gm->CanMove(randomX, randomY, randomX, randomY); // turns false if spawnw location is valid
			...
			monster->cellX = monster->targetCellX = monster->spawnX = randomX;
			monster->cellY = monster->targetCellY = monster->spawnY = randomY;
			monster->SpecialAI = TRUE;										  // Shiny Unicorn has an AI.
			gm->mobList->Add(monster);
			UnicornLocations[monster->subType][0] = randomX;
			UnicornLocations[monster->subType][1] = randomY;
...
//							badPlace = TRUE; // can't be by the ROP tower.
						...
						while (curMobbadplace)
						{
							...
							if (abs(tm->enterX - curMobbadplace->cellX) < 4 && abs(tm->enterY - curMobbadplace->cellY) < 4)
							{
                ...
                if (curMob->isDead && SMOB_MONSTER == curMob->WhatAmI())
                {
					...
					if (((BBOSMonster *)curMob)->SpecialAI)
					{
						...
						if (((BBOSMonster *)curMob)->type == 31)
						{
							...
							// now we are good, and can spawn the new Unicorn, but only in the main map.
							if (sp->WhatAmI() == SPACE_GROUND)
							{
								...
								sp->mobList->Add(monster);											// add it to the moblist.
							}
						}
						// no other AIs, yet.
					}
             ...
             transferRequestPtr->ptr)
        {
            ...
            if (io) // if found the object to transfer
            {
                          ...
                          avatar->charInfoArray[avatar->curCharacterIndex].inventory->money)
                {
					...
					else if (avatar->charInfoArray[avatar->curCharacterIndex].beast > 9 && !strcmp(io->do_name, "Dodging"))
					{
...
int BBOServer::ShiftItem(BBOSAvatar *avatar, MessInventoryChange *inventoryChangePtr)
{
	...
	if (io) // if found the object to transfer
	{
		...
		if ((GTM_BUTTON_LIST_WLD == inventoryChangePtr->dstListType) && // if it was a wield action
			...
			(avatar->BeastStat() > 9))									// if character is a beastmaster
		{
...
void BBOServer::HandleWordOfPower(BBOSAvatar *ca, SharedSpace *sp) 
{
	...
	int uidcount = 0;
	BBOSMonster *savedmonster = NULL; // needs to be here for use by power words.
    ...
    {
		// am i controlling a monster as a beastmaster? 
		if (ca->controlledMonster)
		{
			// go to follow mode HACK
			ca->followMode = TRUE;
			// and send rlease message ot move camera back to client.
					// i need to stash my monster
			MessAdminMessage adminMess; // create an admin message to tell client that it's no lnoger controlling a monster so it doesn't get confused by the map change.
			adminMess.messageType = MESS_ADMIN_RELEASE_CONTROL;
			lserver->SendMsg(sizeof(adminMess), (void *)&adminMess, 0, &tempReceiptList);
			savedmonster = ca->controlledMonster;
			// first check follow mode.
			 if (ca->followMode)
			 {
				// first part - before dissapearing
					MessMonsterDeath messMD;
					...
					ca->controlledMonster = NULL; // unconfuse the client.
			}
		}
		if (ca->controlledMonster && !ca->followMode) // for reto with monster out and about
		{
			// and make my monster vanish too
			MessMonsterDeath messMD;
			messMD.mobID = (unsigned long)ca->controlledMonster;
		...
		ca->cellY = ca->targetCellY =
			((BBOSAvatar *)ca)->charInfoArray[((BBOSAvatar *)ca)->
			curCharacterIndex].spawnY;    MessChangeMap changeMap;
		SharedSpace *sg = (SharedSpace *)spaceList->Find(SPACE_GROUND);
		if (ca->controlledMonster && !ca->followMode)
		{   // set the coordinates here
			ca->controlledMonster->cellX = ca->cellX;
			ca->controlledMonster->cellY = ca->cellY;
			ca->controlledMonster->targetCellX = ca->cellX;
			ca->controlledMonster->targetCellY = ca->cellY;
			// add the monster
			sg->mobList->Add(ca->controlledMonster);
...
//		if ((SPACE_GROUND == sp->WhatAmI()) && (((GroundMap*)sp)->type > 0))
        ...
        ca->IntroduceMyself(sg, SPECIAL_APP_DUNGEON);
		// second part. after introducingmyself
		ca->controlledMonster = savedmonster;
		if (ca->controlledMonster)  // if we had a monster, spawn it again like i logged in. 
		{
			if (ca->followMode)
			{
				...
				ca->controlledMonster->targetCellX = ca->cellX;
				ca->controlledMonster->targetCellY = ca->cellY;
				ca->controlledMonster->isMoving = TRUE;
				ca->controlledMonster->moveStartTime = timeGetTime() - 10000;
				sg->mobList->Add(ca->controlledMonster);
				MessMobAppear mobAppear;

				mobAppear.mobID = (unsigned long)ca->controlledMonster;
				mobAppear.type = ca->controlledMonster->WhatAmI();
				mobAppear.monsterType = ca->controlledMonster->type;
				mobAppear.subType = ca->controlledMonster->subType;

				mobAppear.x = ca->controlledMonster->cellX;
				mobAppear.y = ca->controlledMonster->cellY;
				sg->SendToEveryoneNearBut(0, ca->controlledMonster->cellX, ca->controlledMonster->cellY, sizeof(mobAppear), &mobAppear);
				ca->controlledMonster->AnnounceMyselfCustom(sg);
			}
			else // still need to relocate the monster 
			{
				// tell clients about my monster
				ca->controlledMonster->AnnounceMyselfCustom(sg);
				// tell client where ithe controlled monster is
				MessMobBeginMoveSpecial bMoveSpecial;
				bMoveSpecial.mobID = (unsigned long)ca->controlledMonster;
				bMoveSpecial.x = ca->controlledMonster->cellX;
				bMoveSpecial.y = ca->controlledMonster->cellY;
				bMoveSpecial.targetX = ca->controlledMonster->cellX;
				bMoveSpecial.targetY = ca->controlledMonster->cellY;
				bMoveSpecial.specialType = 1;  // controlled monster
				sg->lserver->SendMsg(sizeof(bMoveSpecial),
					(void *)&bMoveSpecial,
					0, &tempReceiptList);
			}
		}
